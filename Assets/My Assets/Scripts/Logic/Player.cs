...
public class Player : MonoBehaviour, ICharacter
{
	...
	public SpellEffect HeroPower2Effect;
    // a flag not to use hero power twice
    public bool usedHeroPowerThisTurn = false;
	public bool usedHeroPower2ThisTurn = false;
	...
	{
		ManaLeft -= 2;
		usedHeroPower2ThisTurn = true;
		HeroPower2Effect.ActivateEffect();
	}
	}
    ...
    public void HighlightPlayableCards(bool removeAllHighlights = false)
    {
		...
		// highlight second heroPower
		PArea.HeroPower2.Highlighted = (!usedHeroPower2ThisTurn) && (ManaLeft > 1) && !removeAllHighlights;    
	}
    ...
    {
        Health = charAsset.MaxHealth;
		Armor = charAsset.Armor;
        // change the visuals for portrait, hero power, etc...
        PArea.Portrait.charAsset = charAsset;
        PArea.Portrait.ApplyLookFromAsset();
		if (charAsset.Armor == 0) {
			PArea.Portrait.ArmorIcon.SetActive (false);
		}
		...
		if (charAsset.HeroPower2Name != null && charAsset.HeroPower2Name != "")
		{
			HeroPower2Effect = System.Activator.CreateInstance(System.Type.GetType(charAsset.HeroPower2Name)) as SpellEffect;
		}
		else
		{
			Debug.LogWarning("Check hero powr 2 name for character " + charAsset.ClassName);
		}
    }
